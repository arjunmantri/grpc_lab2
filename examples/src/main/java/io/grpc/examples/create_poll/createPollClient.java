package io.grpc.examples.create_poll;
...
public class createPollClient{
    private static final Logger logger = Logger.getLogger(createPollClient.class.getName());
 
    private final ChannelImpl channel;
    private final PollServiceGrpc.PollServiceBlockingStub blockingStub;
 
    public createPollClient(String host, int port) {
        channel =
    ...
    public static void main(String[] args) throws Exception {
        createPollClient client = new createPollClient("localhost", 50051);
        try {
